...
try {
	...
	$sql = "SELECT id, date_effective, length_type_id FROM policies WHERE renewal = 0 AND date_effective IS NOT NULL;";
	...
	foreach ($result as $renewal_info) {
		...
		// only select expired dates
		if ($expire_date <= $today) {
			// mark id as pending renewal
			$ids[] = $renewal_info->id;
		}
	...
	if (count($ids) > 0) {
		// put ids in a comma delimited list
		$policy_ids = implode(",",$ids);
	
		// update query to mark pending renewal
		$query_pending_renewal = $dbh->prepare('UPDATE policies SET renewal = 1 WHERE id IN ('.$policy_ids.');');
